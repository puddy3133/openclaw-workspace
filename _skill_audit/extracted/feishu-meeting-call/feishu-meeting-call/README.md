...
|------|------|------|
| `im:message` | 机器人发送消息 | ✅ |
| `im:message:urgent` | 应用内加急（免费） | ✅ |
| `im:message.urgent:phone` | 电话加急（消耗企业配额） | 用 `--phone-call` 时需要 |
| `contact:user.id:readonly` | 通过邮箱/手机号查找用户 | 用 `lookup` 命令时需要 |
...
export FEISHU_USER_OPEN_ID="被提醒用户的 open_id"
```
...
将获取到的 `open_id` 设置为 `FEISHU_USER_OPEN_ID` 环境变量。
...
| 错误码 | 原因 | 解决方法 |
|--------|------|---------|
| 230027 | 缺少加急权限 | 权限管理 → 开通 `im:message:urgent` 或 `im:message.urgent:phone` |
