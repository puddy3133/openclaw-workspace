...
source ~/.feishu.env && python3 {baseDir}/scripts/feishu_meeting.py lookup --phone "用户的手机号"
```
...
然后**继续执行用户原本的请求**（发提醒、打电话等），执行命令前先 `source ~/.feishu.env` 确保变量可用：

```bash
source ~/.feishu.env && python3 {baseDir}/scripts/feishu_meeting.py notify --message "xxx"
```
...
5. 左侧「权限管理」→ 搜索并开通以下权限：
   ...
   - `im:message.urgent:phone`（电话加急，可选）
   - `contact:user.id:readonly`（查找用户，可选）
...
- 建议：请在飞书开放平台开通 im:message 和 im:message:urgent 权限
```
...
| `im:message:urgent` | 应用内加急 | 默认加急方式（免费） | 是 |
| `im:message.urgent:phone` | 电话加急 | `--phone-call` 时使用（消耗企业配额） | 用到电话加急时必需 |
| `contact:user.id:readonly` | 通过邮箱/手机号查找用户 | lookup 命令 | 用到 lookup 时必需 |
...
| 加急失败 (230027) | 缺少加急权限 | 权限管理 → 开通 `im:message:urgent` 或 `im:message.urgent:phone` |
