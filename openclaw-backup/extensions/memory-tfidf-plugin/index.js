...
function loadConfig() {
  try {
    ...
    if (fs.existsSync(configPath)) {
      ...
      return (
        config.plugins?.entries?.['memory-tfidf-plugin']?.config ||
        config.plugins?.entries?.['memory-tfidf']?.config ||
        {}
      );
    }
  } catch (err) {
...
// Session cache to avoid re-reading files on every message
let _sessionPromptCache = null;
let _sessionPromptCacheTime = 0;
let _sessionPromptCacheKey = '';
const SESSION_CACHE_TTL_MS = 60 * 1000; // 1-minute TTL

function extractUserQuery(event) {
  const candidates = [];
  if (event && typeof event.userMessage === 'string') candidates.push(event.userMessage);
  if (event && typeof event.input === 'string') candidates.push(event.input);
  if (event && typeof event.message === 'string') candidates.push(event.message);
  ...
  const query = candidates.find(x => x && x.trim().length > 0) || '';
  return query.trim();
}

function buildSystemPrompt(basePrompt, event = null) {
  const store = getMemoryStore();
  const config = { ...DEFAULT_CONFIG, ...loadConfig() };
  const query = extractUserQuery(event);
  const cacheKey = query.slice(0, 200);

  // Use cached context within the same session to avoid repeated file I/O
  const now = Date.now();
  if (
    _sessionPromptCache &&
    _sessionPromptCacheKey === cacheKey &&
    (now - _sessionPromptCacheTime) < SESSION_CACHE_TTL_MS
  ) {
    return (basePrompt || '') + _sessionPromptCache;
  }

  let extra = '';

  // Inject index-sized long-term memory (expect concise MEMORY.md index).
  const evergreen = store.loadEvergreen();
  if (evergreen) {
    const header = evergreen.slice(0, 1200);
    extra += `\n\n---\n\n## Long-term Memory (brief)\n\n${header}`;
  }

  // Only inject today's and yesterday's log, at most 200 chars each
  const recent = store.getRecentMemories(config.maxRecentDays);
  if (recent.length > 0) {
    extra += '\n\n---\n\n## Recent Memory (last 2 days)';
    for (const entry of recent.slice(0, 2)) {
      const snippet = entry.content.slice(0, config.maxSnippetLength);
      extra += `\n\n### ${entry.date}\n${snippet}`;
    }
  }

  // Query-aware retrieval: append the most relevant memory chunks for current user request.
  if (query.length >= (config.queryMinLength || DEFAULT_CONFIG.queryMinLength)) {
    try {
      const hits = store.searchMemory(query, config.topK).slice(0, config.topK || 3);
      if (hits.length > 0) {
        extra += '\n\n---\n\n## Related Memory Hits';
        for (const hit of hits) {
          extra += `\n\n- ${hit.path}:${hit.lineStart} (score=${hit.score.toFixed(3)})\n  ${hit.snippet}`;
        }
      }
    } catch (err) {
      console.error('[memory-tfidf] searchMemory failed:', err.message);
    }
  }

  _sessionPromptCache = extra;
  _sessionPromptCacheTime = now;
  _sessionPromptCacheKey = cacheKey;
...
async function beforeAgentStartHandler(event, ctx) {
  ...
  try {
    const enhancedPrompt = buildSystemPrompt(event.systemPrompt, event);
...
function register(api) {
  ...
  if (api.registerHook) {
    try {
      api.registerHook('before_agent_start', beforeAgentStartHandler, { name: 'memory-tfidf-plugin' });
