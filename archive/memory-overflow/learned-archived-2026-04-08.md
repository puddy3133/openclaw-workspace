...
- [2026-04-07] **Agent工作流设计**：Superpowers插件的14个Skills工作流值得借鉴——规划2小时、执行10分钟、审查1小时。苏格拉底式提问（一次一个问题）比并行提问更能挖掘真实需求。
- [2026-04-07] **任务拆解原则**：将开发任务拆成2-5分钟可完成的小任务，每完成一个就验证，出问题马上发现，避免项目写完才爆炸。
- [2026-04-07] **分层审查机制**：局部审查（功能实现）+ 全局审查（模块集成）+ 代码质量审查，三道关卡确保产出质量。
- [2026-04-07] **版本隔离最佳实践**：使用git worktree创建隔离工作区，分支开发不影响主分支，做完再合并。

- [2026-04-07] **Prompt心法-角色选择**: 不确定选什么角色时，让AI自己选顶尖专家（如活动策划选Priya Parker），比用户指定更精准。
- [2026-04-07] **Prompt心法-苏格拉底追问**: 95%置信度作为停止条件，既保证质量又避免无尽循环。一次只问一个问题，根据回答继续追问。
- [2026-04-07] **Prompt心法-反对者角色**: 主动要求AI扮演反对者攻击你的观点，打破AI谄媚效应，获得客观判断。
- [2026-04-07] **Prompt心法-预演失败**: 项目开始前假设失败并复盘，识别认知盲区（如排队、盒饭、厕所、动线等细节风险）。
- [2026-04-07] **Prompt心法-反向提示**: 看到好内容时，给成品让AI倒推Prompt，学习结构和节奏。
