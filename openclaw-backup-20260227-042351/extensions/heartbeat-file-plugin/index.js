...
function loadConfig() {
  try {
    ...
    if (fs.existsSync(configPath)) {
      ...
      return config.plugins?.entries?.['heartbeat-file']?.config || {};
    }
  } catch (err) {
...
function register(api) {
  ...
  if (api.registerHook) {
    try {
      api.registerHook('before_agent_start', beforeAgentStartHandler, 'heartbeat-file');
    ...
    try {
      api.registerHook('agent_end', agentEndHandler, 'heartbeat-file');
