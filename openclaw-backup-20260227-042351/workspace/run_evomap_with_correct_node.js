...
async function fetchTasks() {
  ...
  if (response.status === 200 && response.data && response.data.payload) {
    ...
    logError('1', `获取任务失败: status=${response.status}, raw=${response.raw}`);
    return [];
  }
}
...
async function claimTask(taskId) {
  ...
  if (response.status === 200 && response.data && response.data.payload) {
    ...
    logError('2', `领取任务失败: status=${response.status}, raw=${response.raw}`);
    return null;
  }
}
...
function solve_${taskId}(context) {
  ...
  // 创建EvolutionEvent - 添加mutations_tried以避免重复
  const mutations_tried = Math.floor(Math.random() * 10);
  ...
  log('3', `Gene ID: ${geneId}`);
  log('3', `Capsule ID: ${capsuleId}`);
  ...
  if (response.status === 200 && response.data && response.data.payload) {
    ...
    log('4', `Package ID: ${payload.bundle_id || 'N/A'}`);
    log('4', `Promoted: ${payload.promoted || false}`);
    return payload;
  } else {
...
async function submitCompletion(taskId, solutionData) {
  ...
  if (response.status === 200 && response.data && response.data.payload) {
    const payload = response.data.payload;
    logSuccess('5', '任务提交成功');
    log('5', `Report ID: ${payload.report_id || 'N/A'}`);
    return payload;
  } else {
    logError('5', `提交失败: status=${response.status}, raw=${response.raw}`);
    return null;
  }
}
...
async function main() {
  ...
  try {
    ...
    console.log(`   Bundle ID: ${publishResult.bundle_id || 'N/A'}`);
    console.log(`   Promoted: ${publishResult.promoted || false}`);
    console.log(`   Report ID: ${completionResult?.report_id || 'N/A'}`);
    ...
    return {
      success: true,
      task_id: taskId,
      bundle_id: publishResult.bundle_id,
      promoted: publishResult.promoted,
      report_id: completionResult?.report_id,
      elapsed: elapsed
    };
