...
| 4 | 提示词封装（Prompt Pack） | 把 Copy Spec 封装成可复制提示词；用户确认后通过浏览器 Gemini 出图并回传展示 | `stages/04-prompts.md` |
...
4. Copy Spec 已确认，要出可复制提示词；提示词确认后通过浏览器 Gemini 出图并回传展示 → 阶段4
...
- 提示词确认后：通过浏览器调用用户 Gemini 账号逐张出图，回传图片到对话中展示（阶段4）
